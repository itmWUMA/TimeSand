<template>
  <section class="space-y-6">
    <header class="space-y-2">
      <h1 class="text-3xl font-semibold text-ts-accent">Music & Playlists</h1>
    ...
    <div class="grid gap-6 xl:grid-cols-[1.2fr,1fr]">
      <section class="space-y-3 rounded-xl border border-white/10 bg-ts-panel p-4">
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-semibold text-ts-accent">All Tracks</h2>
        ...
        <ul v-else class="space-y-2">
          ...
          >
            <div class="min-w-0 flex-1">
              <p class="truncate text-sm font-medium text-ts-text">{{ track.title }}</p>
      ...
      <section class="space-y-4">
        <div class="space-y-3 rounded-xl border border-white/10 bg-ts-panel p-4">
          <h2 class="text-xl font-semibold text-ts-accent">Playlists</h2>
...
const addTrack = async (musicId: number): Promise<void> => {
  if (selectedPlaylistId.value == null) {
    return;
  }
...
const removeTrack = async (musicId: number): Promise<void> => {
  if (selectedPlaylistId.value == null) {
    return;
  }
...
const reorderTracks = async (trackIds: number[]): Promise<void> => {
...
const removeMusic = async (musicId: number): Promise<void> => {
  errorMessage.value = null;
  try {
